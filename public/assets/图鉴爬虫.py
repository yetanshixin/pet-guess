...
BASE_URL = "https://wiki.biligame.com/nrc/"
STATS_URL = "https://wiki.biligame.com/nrc/精灵列表"
HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
    'Accept-Language': 'zh-CN,zh;q=0.9',
    'Referer': 'https://wiki.biligame.com/nrc/%E7%B2%BE%E7%81%B5%E5%88%97%E8%A1%A8',
...
def merge_data(new_data, old_data):
    ...
    for key, old_val in old_data.items():
        ...
        if key == '特性':
            ...
            merged[key] = [new_0 if new_0 else old_0, new_1 if new_1 else old_1]
            continue  # 特性单独处理完毕，跳过下方的通用逻辑

        # 嵌套字典（详情/种族/进化/体型/生态/形态）：递归合并子字段
        if isinstance(new_val, dict) and isinstance(old_val, dict):
            merged[key] = merge_data(new_val, old_val)
            continue
...
def get_more_info(url, pet_info, session):
    ...
    图片_list = pet_doc.xpath('//*[@id="rocodex-pAppear"]//div[@class="roco-stage-art"]/div[1]/img/@src')
    if 图片_list:
    ...
    if pet_info['进化']['阶段'] > 1:
        当前阶段_list = pet_doc.xpath(
            f'//*[@id="rocodex-pEvo"]/div/div/div[@class="roco-evo-conn"]')
        if 当前阶段_list and 当前阶段_list[pet_info["进化"]["阶段"] - 2] is not None:
            当前阶段 = 当前阶段_list[pet_info["进化"]["阶段"] - 2]
            当前进化_list = 当前阶段.xpath('./div/span[2]/text()')
            if 当前进化_list:
                当前进化_str = 当前进化_list[0].strip()
                arr = 当前进化_str.split(' ', 1)
                if arr:
    ...
    if 雄性占比_list:
        雄性占比_str = re.search(r'\d+', 雄性占比_list[0].strip())
        if 雄性占比_str: pet_info['生态']['雄性占比'] = int(雄性占比_str.group())
...
def main():
    ...
    pet_list = document.xpath('//*[@id="mw-content-text"]//table/tbody/tr[@class="npc-table-row"]')
    all_pet = []
    ...
    for pet in pet_list:
        pet_info = {
            '编号': 0, '名称': '', '属性': [],
            '详情': {'描述': '', '介绍': '', '特性': ['', '']},
            '种族': {'总种族值': 0, '生命': 0, '物攻': 0, '魔攻': 0, '物防': 0, '魔防': 0, '速度': 0},
            '进化': {'阶段': 0, '进化等级': 0, '进化方式': '', '进化链': 0, '是初始阶段': False, '是最终阶段': False},
            '体型': {'身高': [], '体重': []},
            '生态': {'蛋组': [], '雄性占比': None, '可同乘': False},
            '形态': {'有地区形态': False, '有首领形态': False, '有异色形态': False, '异色赛季': 0}
        }

        编号_list = pet.xpath('./@data-number')
        if not 编号_list: continue
        编号_str = re.search(r'\d+', 编号_list[0].strip())
        if not 编号_str: continue
        编号 = int(编号_str.group())

        进化链_list = pet.xpath('./@data-search-ref')
        if not 进化链_list: continue
        进化链_str = re.search(r'\d+', 进化链_list[0].strip())
        if not 进化链_str: continue
        进化链 = int(进化链_str.group())

        形态_list = pet.xpath('./@data-form')
        形态 = "原始形态"
        if 形态_list and 形态_list[0].strip() == "regional":
            形态 = "地区形态"
        elif 形态_list and (形态_list[0].strip() == "lord" or 形态_list[0].strip() == "lord|regional"):
            形态 = "首领形态"

        if 形态 == "原始形态":
            pet_info['编号'] = 编号
            pet_info['进化']['进化链'] = 进化链
            头像_list = pet.xpath('./td[2]/img/@src')
            if 头像_list:
                img_url = 头像_list[0].strip()
                save_path = f"{AVATAR_DIR}/{编号}.png"
                download_image(img_url, save_path, session)

            阶段_list = pet.xpath('./@data-stage')
            if 阶段_list:
                pet_info['进化']['阶段'] = int(阶段_list[0].strip())

            位置_list = pet.xpath('./@data-position')
            if 位置_list:
                位置_str = 位置_list[0].strip()
                if 位置_str == "initial|final":
                    pet_info['进化']['是初始阶段'] = True
                    pet_info['进化']['是最终阶段'] = True
                elif 位置_str == "initial|" or 位置_str == "initial":
                    pet_info['进化']['是初始阶段'] = True
                elif 位置_str == "final" or 位置_str == "|final":
                    pet_info['进化']['是最终阶段'] = True

            属性_list = pet.xpath('./@data-type')
            if 属性_list:
                属性_str = 属性_list[0].strip()
                pet_info['属性'] = 属性_str.split('|')

            异色_list = pet.xpath('./@data-shiny')
            if 异色_list: pet_info['形态']['有异色形态'] = (异色_list[0].strip() == "yes")

            异色赛季_list = pet.xpath('./@data-season')
            if 异色赛季_list:
                异色赛季_str = re.search(r'\d+', 异色赛季_list[0].strip())
                if 异色赛季_str:
                    pet_info['形态']['异色赛季'] = int(异色赛季_str.group())

            特性名_list = pet.xpath('./td[5]/div/a/text()')
            if 特性名_list: pet_info['详情']['特性'][0] = 特性名_list[0].strip()
            特性_list = pet.xpath('./td[13]/text()')
            if 特性_list: pet_info['详情']['特性'][1] = 特性_list[0].strip()

            pet_info['种族']['生命'] = safe_int(pet.xpath('./td[6]/text()'))
            pet_info['种族']['速度'] = safe_int(pet.xpath('./td[7]/text()'))
            pet_info['种族']['物攻'] = safe_int(pet.xpath('./td[8]/text()'))
            pet_info['种族']['魔攻'] = safe_int(pet.xpath('./td[9]/text()'))
            pet_info['种族']['物防'] = safe_int(pet.xpath('./td[10]/text()'))
            pet_info['种族']['魔防'] = safe_int(pet.xpath('./td[11]/text()'))
            pet_info['种族']['总种族值'] = safe_int(pet.xpath('./td[12]/text()'))

            蛋组_list = pet.xpath('./td[17]/text()')
            if 蛋组_list:
                蛋组_str = 蛋组_list[0].strip()
                if 蛋组_str != '未发现':
                    pet_info['生态']['蛋组'] = 蛋组_str.split(' / ')

            身高_list = pet.xpath('./td[18]/text()')
            if 身高_list:
                身高_str = 身高_list[0].strip()
                pet_info['体型']['身高'] = parse_range(身高_str)

            体重_list = pet.xpath('./td[19]/text()')
            if 体重_list:
                体重_str = 体重_list[0].strip()
                pet_info['体型']['体重'] = parse_range(体重_str)

            同乘_list = pet.xpath('./td[21]/span/@title')
            if 同乘_list:
                pet_info['生态']['可同乘'] = (同乘_list[0].strip() == '可同乘')

            介绍_list = pet.xpath('./td[22]/text()')
            if 介绍_list: pet_info['详情']['介绍'] = 介绍_list[0].strip()

            名称_list = pet.xpath('./td[3]/a/@title')
            if 名称_list:
            ...
            if 进化链:
                arr = [item for item in all_pet if item['进化']['进化链'] == 进化链]
                if arr:
                    for p in arr:
                        if 形态 == "首领形态":
                            p['形态']['有首领形态'] = True
